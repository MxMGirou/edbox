...
function setFacture($id_user,$id_plan)
{
	...
	$req = $bdd->prepare('SELECT prix_an FROM edbox_plan WHERE id_plan = :id');
	...
	//on creer la facture
	$req = $bdd -> prepare ('INSERT INTO edbox_facture(	date_emission, date_expiration, montant, id_user, id_plan, etat) VALUES(?,?,?,?,?,?)');
	$req -> execute (array(
	$date[0],
	$date[1],
	$prix_plan,
